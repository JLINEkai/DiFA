...
from diffusion.BrownianBridge.BrownianBridgeModel import BrownianBridgeModel
...
class LatentBrownianBridgeModel(BrownianBridgeModel):
    ...
    @torch.no_grad()
    def sample(self, x_cond, y, clip_denoised=False, sample_mid_step=False):
        # x_cond_latent = self.encode(x_cond, cond=True)
        if sample_mid_step:
            temp, one_step_temp = self.p_sample_loop(y=x_cond,
                                                     context=self.get_cond_stage_context(x_cond),
            ...
            temp = self.p_sample_loop(x_cond, y,
                                      sample_mid_step=sample_mid_step)
            out = temp
