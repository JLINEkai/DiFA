...
from diffusion.BrownianBridge.base.modules.diffusionmodules.openaimodel import UNetModel
from diffusion.BrownianBridge.base.modules.encoders.modules import SpatialRescaler
...
class BrownianBridgeModel(nn.Module):
    def __init__(self, model_config):
        ...
        model_params = model_config.model.BB.params
        ...
        self.denoise_fn = DiT_models[model_config.model.dit_model](
            input_size=model_config.model.latent_size,
    ...
    def forward(self, x, y, label, t, context=None):
        ...
        return self.p_losses(x, y, t, label)

    def p_losses(self, x0, y, t, label, noise=None):
        """
        model loss
        ...
        objective_recon = self.denoise_fn(x_t, t, label)
        B, C = x_t.shape[:2]
        assert objective_recon.shape == (B, C * 2, *x_t.shape[2:])
        objective_recon, model_var_values = torch.split(objective_recon, C, dim=1)
    ...
    @torch.no_grad()
    def p_sample(self, x_t, y, label, context, i, clip_denoised=False):
    ...
    @torch.no_grad()
    def p_sample_loop(self, y, label, context=None, clip_denoised=True, sample_mid_step=False):
        if self.condition_key == "nocond":
            context = None
        else:
            context = y if context is None else context
        ...
        if sample_mid_step:
            ...
            img = y
            for i in tqdm(range(len(self.steps)), desc=f'sampling loop time step', total=len(self.steps)):
                img, _ = self.p_sample(x_t=img, y=y, label = label, context=context, i=i, clip_denoised=clip_denoised)
            return img
