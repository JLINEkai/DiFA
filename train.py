...
class CustomDataset(Dataset):
    def __init__(self, features_dir, labels_dir):
        ...
        labels = []
        i = 0
        for x in ['HER2', 'ER', 'Ki67', 'PR']:
            i += 1
            self.he = np.load(features_dir.replace('HER2', x))
            self.ihc = np.load(labels_dir.replace('HER2', x))
            features.append(self.he)
            features.append(self.ihc)
            labels.append(([0]*self.he.shape[0]))
            labels.append(([i]*self.ihc.shape[0]))
...
def main(args):
    ...
    features_dir = "./combined_mist_he_train_PR_consistency.npy"
    labels_dir = "./combined_mist_ihc_train_PR_consistency.npy"
...
# accelerate launch --multi_gpu --num_processes 8 train.py --model DiT-S/2 